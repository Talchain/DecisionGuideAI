 ...
 * provenance-flagged, check affordances inline. Effect-strength values stay
 * gated (value-scale fix pending); the gate line says so.
 *
 * Alignment grid: every group row starts on the same 24px-inset edge (the
 * shape column), pills and meta live on row line 2 or the right edge only.
...
const GroupHeader = memo(function GroupHeader({
  ...
  return (
    <>
      ...
      <p className={`${typography.panelMeta} mb-1 ml-6 mt-1 text-text-light`}>{coach}</p>
    </>
  )
})
...
export const YourDecisionSection = memo(function YourDecisionSection({
  ...
  // Ladder "check estimate" action: open the section, expand and reveal the row.
  useEffect(() => {
