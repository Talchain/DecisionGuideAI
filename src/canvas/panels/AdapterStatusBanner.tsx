...
export function AdapterStatusBanner({ visible = true }: AdapterStatusBannerProps): JSX.Element | null {
  ...
  return (
    <div className="mx-4 mt-4 p-3 bg-panel border border-warning/30 rounded-lg">
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`${typography.panelBody} text-warning mt-1`}>
            Using mock adapter until backend deploys <code className="bg-panel px-1 py-0.5 rounded text-xs">/v1/run</code> and <code className="bg-panel px-1 py-0.5 rounded text-xs">/v1/stream</code> endpoints.
          </p>
          <div className={`flex items-center gap-3 mt-2 ${typography.panelMeta} text-warning`}>
        ...
        <button
          ...
          disabled={reprobing}
          className={`flex items-center gap-1.5 px-2 py-1 ${typography.panelMeta} text-warning hover:text-warning hover:bg-panel rounded transition-colors disabled:opacity-50`}
