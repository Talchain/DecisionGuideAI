      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-2">
            <h3 className={`${typography.panelHeader} text-info-900`}>
              About {blueprint.name}
            </h3>
            {/* v1.2: Version chip */}
            {normalizedVersion && (
              <span className={`flex-shrink-0 px-2 py-0.5 ${typography.panelMeta} rounded-full bg-info-100 text-info-700 border border-info-200`}>
                v{normalizedVersion}
              </span>
            )}
          </div>
          <p className={`${typography.panelBody} mb-3 text-gray-700`}>
          ...
          {Object.keys(kindCounts).length > 0 && (
            <div className="mb-3">
              <p className={`${typography.panelBody} mb-1.5 text-info-900`}>Node types:</p>
              <div className="flex flex-wrap gap-1.5">
                {Object.entries(kindCounts).map(([kind, count]) => (
                  <span
                    key={kind}
                    className={`inline-flex items-center gap-1 px-2 py-0.5 ${typography.panelMeta} rounded-full bg-gray-100 text-gray-700 border border-gray-200`}
                  >
                    {kind}: {count}
                  </span>
                ))}
          ...
          {bodyPreview && (
            <div className="mb-3 p-2 rounded bg-gray-50 border border-gray-200">
              <p className={`${typography.panelBody} mb-1 text-gray-700`}>Example detail:</p>
              <p className={`${typography.panelBody} text-gray-600 italic`}>"{bodyPreview}"</p>
            </div>
          )}

          {blueprint.expectedInputs && blueprint.expectedInputs.length > 0 && (
            <div className="mb-2">
              <p className={`${typography.panelBody} mb-1 text-info-900`}>Expected inputs:</p>
              <p className={`${typography.panelBody} text-gray-600`}>
          ...
          {blueprint.assumptions && blueprint.assumptions.length > 0 && (
            <div>
              <p className={`${typography.panelBody} mb-1 text-info-900`}>Assumptions:</p>
              <p className={`${typography.panelBody} text-gray-600`}>
