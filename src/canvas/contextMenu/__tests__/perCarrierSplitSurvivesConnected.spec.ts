...
function entries(): MenuEntry[] {
  return [
    ...
    { id: 'paste', label: 'Paste', enabled: true, action: () => {} },
...
/**
 * ⚠ `paste` is KEYBOARD-REACHABLE, so an unauthorised `paste` is SURFACED AND
...
describe('the per-carrier split survives an injected connected=true', () => {
  ...
  it('OPPOSITE DIRECTION: admits the durable-add id when its own carrier is live', () => {
    ...
    expect(actionable(out, 'paste'), 'carrierless id inert in the same menu').toBe(false)
  })
  ...
  it('the injected value drives the carrierless id, not the module constant', () => {
    ...
    expect(actionable(shut, 'paste'), 'connected:false ⇒ inert').toBe(false)
    expect(actionable(open, 'paste'), 'connected:true ⇒ actionable').toBe(true)
  })
})
