...
/** The first unconfirmed structural edit still on the canvas — renames before adds, as before. */
function unresolvedStructuralEditOnCanvas(
  state: EditDeliveryState,
): { edit: 'rename' | 'add'; nodeId: string } | null {
 ...
 * `editDeliveryHold` is its `cause`. Pure over its argument plus the two
 * module-level registers it names.
 */
