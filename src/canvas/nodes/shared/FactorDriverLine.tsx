 ...
 * ── WHAT THE `of M` MEANS ───────────────────────────────────────────────────
 *
 * ⭐⭐ CONTRACT v3.1 POINT 5 (DESIGN-GAP-v31 #37, Canvas WS4, 26 Sep): "Driver
 * N of M ranked in this run", M = the RANKED count (`driverRankFor`'s
 * `setSize`). ⚠ This reverses ED #63 5806207128 (24 Sep), which had restored
 * the v3 definition ("Denominator = eligible analysed factors, not 'number of
 * ranks we happen to render'") — named in `DRIVER_LINE_COPY.rank`; one
 * droppable commit. The history below is kept as the record.
 *
 ...
 * may carry no rank. Contract v3.1 pt 5 (#37): `M` is the caption's own
 * `rank.setSize`, the factors the run RANKED; the cap is the owner's
 * `MAX_BADGED_RANK`. No other number is stated. "the run" is true of a current
 * run and of a known-changed model's last one, so the note has one form.
 */
export function driverLineDenominatorNote(rank: FactorDriverLineProps['rank']): string {
  return `“of ${rank.setSize}” counts the factors the run ranked. Olumi names a driver rank for at most ${MAX_BADGED_RANK} factors, and only where their order is clear, so a factor without a rank has not been left out.`
}
