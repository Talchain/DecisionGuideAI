 ...
 * the card's own words — `Driver N of M ranked in this run` (v3.1 pt 5) — and
...
describe('the deployed defect: a factor with no stated value said nothing', () => {
  ...
  it('v3.1 pt 5 stale form — "Last run · Driver N of M ranked"', () => {
    expect(
      resolveLodMetricLine({
        nodeType: 'factor',
        data: { label: 'Team capacity' },
...
describe('the pre-analysis arms, and the opposite-direction twin for each', () => {
  ...
  it('TWIN — the driver rank still outranks the range, so no post-analysis card changes', () => {
    // Locked Canvas design (23 Sep 2026): the rank is `Driver N of M ranked in this run`
