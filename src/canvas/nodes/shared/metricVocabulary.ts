...
export const DRIVER_LINE_COPY = {
  // ⭐ CONTRACT v3.1 POINT 5 (design-gap row 39): "“Driver N of M ranked in this
  // run”, where M is the number of factors the run ranked … Stale form: “Last
  // run · Driver N of M ranked”." This REVERSES ED #63 5806207128 (24 Sep,
  // `Driver N of M analysed`, M = the analysed set) on the brief's instruction
  // that v3.1 pt 5 governs. The words and the number move TOGETHER: "ranked in
  // this run" over the analysed count would claim ranks that were never given,
  // so M is `rankFactor`'s `rankedSetSize` at every printing surface
  // (`driverRankFor`, `nodeAttention`). The caller owns `LAST_RUN_PREFIX`;
  // `fromLastRun` drops "in this run", mirroring `notRanked` below.
