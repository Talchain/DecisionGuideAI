...
export const OptionNode = memo((props: NodeProps) => {
  ...
  /**
   * A change row's whole sentence — the row's hover. The `est.` mark carries its
   * own note (`OPTION_ROW_ESTIMATE_TITLE`), so the sentence does not repeat it.
   */
  const changeRowSentence = (r: OptionChangeRow): string => [
  ...
  /**
   * ⭐ THE BASELINE'S SECOND LINE (prototype: "Reference for the other
   * alternatives.") — stated only when it is TRUE OF THE DATA: this option is
   * the DECLARED baseline (`is_baseline: true`, the flag
   * `resolveBaselineOptionReference` requires; a label like "Status quo" is not
   * a declaration) and at least one other option exists for it to be the
   * reference of. The option's own description, when it carries one, is the line
   * instead — one differentiator line, never two.
   */
  const baselineIsDeclaredReference =
    isBaselineOption &&
    props.data?.is_baseline === true &&
    nodes.some(n => n.id !== props.id && (n.type === 'option' || n.data?.type === 'option'))
  const baselineReferenceLine = baselineIsDeclaredReference && !ownDifferentiator ? (
    <p
      className={`${typography.edgeLabel} !leading-tight mt-1 m-0 text-text-light`}
      data-testid={`option-baseline-reference-${props.id}`}
    >
      Reference for the other alternatives.
    </p>
  ) : null
  ...
  /**
   * The computed differentiator's full sentence, at the top of the option's
   * popover in BOTH phases (Standard view). The change rows and the baseline
   * meta are back ON the card (prototype, Paul 25 Sep), so they are not repeated
   * here. Placed at the popover call sites rather than inside
   * `preAnalysisPopoverContent` / `layer2Content`, whose several return branches
   * would each need a copy.
   */
  const restingDetailInPreview = !isDetailed && differentiatorRenders ? (
