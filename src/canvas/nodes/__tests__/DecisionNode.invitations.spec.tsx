...
describe('DecisionNode — invitations in Standard view', () => {
  ...
  it('"What could go wrong?" is off the Standard face (the icon asks the FIRST chip) and still reachable in Detailed', () => {
    ...
    expect(within(screen.getByTestId('decision-node-popover')).queryByText('What could go wrong?')).toBeNull()
    cleanup()
