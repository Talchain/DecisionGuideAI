...
describe('the anchor node surfaces the brief it is about', () => {
  ...
  it('truncates at a word boundary and never mid-word', () => {
    setBrief(SCENARIO, BRIEF)
    renderDecision()
  ...
  it('a single unbroken token is never mutilated — height is bounded by the clamp instead', () => {
    // The case the old truncator clipped mid-word. Bounding the WORD is the
    // caller's job only up to the word; bounding the BOX is CSS's.
    const token = 'A'.repeat(ANCHOR_BRIEF_MAX_CHARS + 40)
    setBrief(SCENARIO, token)
    renderDecision()

    const quote = screen.getByTestId('decision-node-brief-text')
    expect(quote.textContent).toBe(token)
