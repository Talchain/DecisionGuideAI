...
/** All text on the card OUTSIDE the (mocked, always-mounted) popover. */
const outsidePopoverText = (): string => {
  const c = document.body.cloneNode(true) as HTMLElement
  c.querySelectorAll('[data-testid="decision-node-popover"]').forEach(n => n.remove())
...
describe('DecisionNode — honest resting state', () => {
  ...
  it('Standard completed run: the invitation AND the resting row are BOTH on the card', () => {
    ...
    // on the card; the chip ROW is off the Standard face. The popover offers
    // the REST of the post-run questions ("Compare options") beside its
    // robustness detail, never the icon's own question again.
    ...
    const popover = screen.getByTestId('decision-node-popover')
    expect(within(popover).getByText('Compare options')).toBeDefined()
    expect(outsidePopoverText()).not.toContain('Compare options')
  ...
  it('Standard completed run with NO stability: the popover keeps its chips, the face carries ONE icon and no chip row, the resting row still renders', () => {
    ...
    // Locked Canvas design (23 Sep 2026): the rail icon asks the FIRST post-run
    // question, so the popover carries the REST ("Compare options") and never
    // repeats the icon's own question.
    const popover = screen.getByTestId('decision-node-popover')
    expect(within(popover).getByText('Compare options')).toBeDefined()
    expect(within(popover).queryByText('Challenge this result')).toBeNull()
    // ED 11:52Z point 1 puts the ONE coaching icon on the face. The face itself
    // carries no chip ROW: outside the popover the question appears once, as
    // the icon.
