 ...
 *
 * ⛔⛔ AND ONE RUN IS DELIBERATELY NOT ADJUDICATED: a bare `→`. It is the new
 * option change row (`option-change-row-*`) rendering neither a from nor a to
 * for a `scale`-unit target whose value the formatter suppresses — the old
 * delta builder WITHHELD such a row (`if (!change.baselineText ||
 * !change.targetText) return null`); `buildOptionChangeRow` does not. That is a
 * suspected PRODUCTION regression, not copy, so it is left to RED this census
 * rather than be written into the allowed set.
 */
const EXPECTED_CENSUS: Record<string, string[]> = {
  ...
  'factor · pre · standard': [],
  'factor · pre · expert': [
    // RESIDUAL — Locked Canvas design (23 Sep 2026), MT-19: a Detailed edge pill
    // for an unconfirmed Olumi strength reads `· Link strength est.` and carries
    // its value on the `title` (no bare-percent pill). Invariant wherever the
    // link is Olumi's estimate, BY RULING: the figure is withheld from the face
    // on purpose, so the words that remain name the quantity and its status.
    '· Link strength est.',
  ],
...
const ADJUDICATED_POSITIONS: Position[] = [
  ...
  // MT-19: the Detailed edge pill's status run for an Olumi-estimated link.
  { what: 'factor · the edge pill `· Link strength est.`', by: 'census', present: (c) => c.querySelector('[data-testid^="edge-pill-strength-estimate-"]') != null },
