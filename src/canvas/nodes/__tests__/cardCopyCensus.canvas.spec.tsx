 ...
 *    legibility floor the caption and its value are ONE leaf (`Scenarios 47%`), so
...
const EXPECTED_CENSUS: Record<string, string[]> = {
  ...
  'option · post · standard': [
    'Scenarios', // CAPTION — `METRIC_NOUN.ahead`, beside the bar and the percentage.
  ...
  // leaf (`Scenarios 47%`), so no invariant caption can be isolated here; a bucket
...
const ADJUDICATED_POSITIONS: Position[] = [
  ...
  { what: 'option · the `Scenarios` caption row', by: 'census', present: byTestId('-win-anchor-option-1') },
...
describe('canvas card copy census (Paul, 31 Aug 2026)', () => {
  ...
  it('the census is exactly what has been adjudicated — nothing added, nothing silently dropped', () => {
    ...
    expect(measured['option · post · standard']).toContain('Scenarios')
