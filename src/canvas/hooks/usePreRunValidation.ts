...
/**
 * A12: an explicit `is_baseline` from the producer wins over the label regex.
 * The regex may only fire when the flag is genuinely ABSENT (`undefined` or
 * `null`) — an explicit `false` is a real fact ("Keep £49 with release" and
 * "Continue Current Staffing" both match the regex's keywords but are NOT
 * the do-nothing option) and must never be treated as unstated.
 *
 * Local to this file because every one of its six call sites reaches this
 * question from a different option shape (raw CEE option, mapped `UIOption`,
 * legacy canvas node) — each resolves its own `explicit` value and shares
 * this one decision.
 */
function isBaselineOption(label: string | undefined, explicit: boolean | null | undefined): boolean {
...
function isCeeOptionResolved(option: {
  ...
  if (isBaselineOption(option.label, option.is_baseline)) return true
  ...
  if (disposition !== 'runnable') {
    ...
    if (!allOptionsResolved) {
      ...
      const anyInterventionsPopulated = isSoftStatus && (ceeAnalysisReady.options?.some(
        o => !isBaselineOption(o.label, o.is_baseline) && Object.keys(o.interventions || {}).length > 0
      ) ?? false)
...
function validateOptions(
  ...
  if (ceeAnalysisReady?.options?.length) {
    ...
    // A12: `ceeOptionToUIOption` does not carry `is_baseline` onto `UIOption`
    // (out of this file's scope to widen), so the explicit flag is read back
    // off the raw CEE options by id wherever a `UIOption` needs the answer.
    const isBaselineById = new Map(
      ceeAnalysisReady.options.map((o) => [o.id, o.is_baseline] as const),
    )
    ...
    const needsMappingOptions = options.filter((o) => {
      ...
      const isBaselineEmpty =
        isBaselineOption(o.label, isBaselineById.get(o.id)) && Object.keys(o.interventions).length === 0
    ...
    const emptyInterventionOptions = options.filter((o) => {
      ...
      return !isBaselineOption(o.label, isBaselineById.get(o.id))
    })
  ...
  // A12: `normaliseOptionFromLegacyNode` does not carry `is_baseline` onto
  // `UIOption` either — read the explicit flag back off the source node's own
  // data, the same field `OptionNode.tsx` and `applyDraftResult.ts` read/write.
  const isBaselineByNodeId = new Map(
    optionNodes.map((n) => [n.id, (n.data as { is_baseline?: boolean | null } | undefined)?.is_baseline] as const),
  )
  ...
  const needsMappingOptions = options.filter((o) => {
    if (o.status !== 'needs_user_mapping') return false
    const isBaselineEmpty =
      isBaselineOption(o.label, isBaselineByNodeId.get(o.id)) && Object.keys(o.interventions).length === 0
    return !isBaselineEmpty
  })
  if (needsMappingOptions.length > 0) {
  ...
  const emptyInterventionOptions = options.filter((o) => {
    if (o.status !== 'ready') return false
    if (Object.keys(o.interventions).length !== 0) return false
    return !isBaselineOption(o.label, isBaselineByNodeId.get(o.id))
  })
  if (emptyInterventionOptions.length > 0) {
    blockers.push({
      code: 'EMPTY_INTERVENTIONS',
