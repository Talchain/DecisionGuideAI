...
interface SignedStrengthSliderProps {
  ...
  value: number
  /** Callback with the new signed value. The canonical store owns the live draft. */
  onChange: (signedValue: number) => void
  /** Start one explicit edit session without creating another value authority. */
  onEditStart?: (signedValue: number) => void
  /** Complete the session with the exact DOM value visible at release/blur. */
  onCommit?: (signedValue: number) => void
  /** Disabled state */
  disabled?: boolean
...
export function SignedStrengthSlider({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Deliberately synchronous. The edge-strength coordinator already owns the
    // 1.5 s wire debounce/coalescing; another timer here would hide a displayed
    // value from the Run barrier and make this control a competing draft owner.
    onChange(newValue)
  }

  const beginEdit = (e: React.SyntheticEvent<HTMLInputElement>) => {
    onEditStart?.(parseFloat(e.currentTarget.value))
  }

  const commitEdit = (e: React.SyntheticEvent<HTMLInputElement>) => {
    onCommit?.(parseFloat(e.currentTarget.value))
  }

  const isNegative = value < 0
  const absValue = Math.abs(value)
  const directionLabel = isNegative ? 'Negative' : value > 0 ? 'Positive' : 'No effect'
  ...
  return (
    <div>
      ...
      <div className="relative h-6 flex items-center">
        ...
        {std != null && std > 0 && (() => {
          const bandLow = Math.max(-1, value - std)
          const bandHigh = Math.min(1, value + std)
        ...
        <input
          ...
          aria-valuemax={1}
          aria-valuenow={value}
          aria-valuetext={`${directionLabel}: ${value.toFixed(2)}`}
