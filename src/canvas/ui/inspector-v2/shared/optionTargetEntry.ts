...
/** Every sentence a refused entry can show — named, so a spec binds the string the reader reads. */
export const OPTION_TARGET_ENTRY_REFUSAL = {
  unreadable: (example: string) =>
    `Not a number this row can read. Enter an amount such as ${example}.`,
  unreadableModelScale: 'Not a number this row can read. Enter a value between 0 and 1.',
  outOfRange: (lowest: string, highest: string) =>
    `Must be between ${lowest} and ${highest} for this factor.`,
  outOfRangeModelScale: 'Must be between 0 and 1 — this row is on the model scale.',
  unitConflict: (unit: string) =>
    `This row is in ${unit} — enter the amount without another unit.`,
  unitOnModelScale: 'This row is on the model scale — enter a plain number between 0 and 1.',
} as const
