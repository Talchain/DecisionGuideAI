...
describe('D2 — a refused option target says it was not saved, and keeps the value as unapplied', () => {
  it('⭐ 422 `system_event_refused_no_write` → "Not saved", on the panel AND the row — RED at a4434670 (silence)', async () => {
    ...
    const notice = await screen.findByTestId('option-intervention-notice')
    expect(notice.textContent ?? '').toMatch(/^Not saved/)
    ...
    expect(notice.textContent ?? '').not.toMatch(/moved on|changed while/i)
    expect(notice.textContent ?? '').not.toMatch(/could not confirm/i)
    // The machine token is never shown to the reader.
    expect(notice.textContent ?? '').not.toMatch(/system_event_refused_no_write/)

    // The value the reader asked for STAYS, marked, on ITS row.
    const marker = screen.getByTestId(`intervention-unapplied-${GDPR}`)
    expect(marker.textContent ?? '').toMatch(/^Not saved: 0\.7/)
  ...
  it('Dismiss puts the SAVED value back and clears the notice', async () => {
    answers.push({ status: 422, body: REFUSED_NO_WRITE_422 })
    renderPanel()
    await typeAndEnter(GDPR, '0.7')
  ...
  it('⭐ the retryable 500 (commit not confirmed) says "Could not confirm", never "Not saved" — RED at a4434670 (silence)', async () => {
    ...
    const notice = await screen.findByTestId('option-intervention-notice')
    expect(notice.textContent ?? '').toMatch(/^Could not confirm/)
    expect(notice.textContent ?? '').not.toMatch(/not saved/i)
    expect(screen.getByTestId(`intervention-unapplied-${GDPR}`).textContent ?? '').toMatch(
      /^Could not confirm: 0\.7/,
    )
  })

  it('CONTRAST — an accepted send shows no notice and no unapplied mark', async () => {
...
describe('D3 — a £ target takes the amount the card shows, and says why when it cannot', () => {
  ...
  it('⭐ an unreadable entry says why, keeps the text, and sends nothing — RED at a4434670 (silent reset)', async () => {
    ...
    expect(refusal.textContent).toBe('Not a number this row can read. Enter an amount such as £60,000.')
  ...
  it('an amount past the factor\'s scale names the range in £ and sends nothing', async () => {
    ...
    const refusal = await screen.findByTestId(`intervention-entry-refusal-${COST}`)
    expect(refusal.textContent).toBe('Must be between £0 and £120,000 for this factor.')
    expect(sentOptionTargetEdits()).toHaveLength(0)
  })
  ...
  it('⭐ a unit typed on a model-scale row is refused WITH a reason, not read as 70 or 0.7', async () => {
    ...
    expect(refusal.textContent).toBe(
      'This row is on the model scale — enter a plain number between 0 and 1.',
    )
    expect(sentOptionTargetEdits()).toHaveLength(0)
  })
})
