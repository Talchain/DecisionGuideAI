...
describe('Replay run (flag-gated)', () => {
  beforeEach(() => {
    vi.resetModules()
    mockFlags({
      isSseEnabled: () => true,
  ...
  it('shows Replay button on Done and replays with same seed; chip appears once', async () => {
    ...
    expect(second.url).toContain('seed=42')

    const chip = screen.getByTestId('replayed-chip')
    expect(!!chip).toBe(true)
  })

  it('flag OFF hides Replay button', async () => {
    vi.resetModules()
    mockFlags({
      isSseEnabled: () => true,
      isParamsEnabled: () => true,
    ...
    // Start and finish
    await screen.findByTestId('panel-root')
    fireEvent.click(screen.getByTestId('start-btn'))
    const first = FakeEventSource.instances[0]
    act(() => { first.emit('open'); first.emit('done') })
