...
describe('expressibility is a STRUCTURAL claim about the contract, not an opinion', () => {
  it('AnalysisStateV1 has no member for enrichment or for the assistant prose — so the three envelope pairs are not statable inside it', () => {
    const members = Object.keys(AnalysisStateV1Schema.shape)
  ...
  it('two pairs ARE statable inside AnalysisStateV1 — both their members are contract members', () => {
    const members = Object.keys(AnalysisStateV1Schema.shape)
    expect(members).toEqual(expect.arrayContaining(['run_state', 'readiness', 'leader_claim', 'usable_for_chips']))
  ...
  it('CX3 is only as expressible as its weakest limb, and the split is recorded per limb', () => {
...
describe('⭐ THE LOAD-BEARING DERIVATION — the contract does NOT already refuse these payloads', () => {
  /**
   * The vendored contract is 0.46.0 and carries ZERO cross-field refinement
   * (its own header: "NO cross-field refinement … stated as LICENCE in a
   * `.describe()` and NOT enforced by the parser"), pinning three DISCLOSED
   * LIMITS. If any of these parsed FALSE, this gate would be duplicating a rule
   * the parser already enforces — and the pair would belong in the schema, not
   * here.
   */
  const contradictoryStates: ReadonlyArray<[string, unknown]> = [
    ...
    ['CX3 never_run + every usability boolean true', {
  ...
  it.each(contradictoryStates.slice(0, 3))('…and the GATE catches what the parser waves through: %s', (_name, payload) => {
