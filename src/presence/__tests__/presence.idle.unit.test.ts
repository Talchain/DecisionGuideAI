...
describe('presence.idle.unit', () => {
  beforeEach(() => { vi.useFakeTimers(); vi.setSystemTime(0) })
  afterEach(() => { try { vi.runOnlyPendingTimers() } catch {}; vi.useRealTimers() })

  it('flips to idle after 45s and back to active on activity', async () => {
    ...
    await act(async () => {
      render(
        React.createElement(FlagsProvider as any, { value: { sandbox: true, sandboxPresence: true } },
          React.createElement(PresenceProvider as any, { decisionId: 'demo', user: { id: 'u1', name: 'Me', color: '#000' }, transport: tx, getEditor: () => editor as any },
    ...
    // Simulate remote activity by publishing an updated payload with a fresh ts
