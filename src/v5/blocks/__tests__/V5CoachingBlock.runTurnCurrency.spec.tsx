...
describe('runTurnStaleReason — which limb failed, with the contract’s precedence', () => {
  ...
  it.each<[string, string | undefined, string | undefined, Partial<RunTurnCurrencyInputs>, RunTurnStaleReason]>([
    ...
    ['run_state is never_run (stated: this model has no run)', HASH, HASH, { runStateKind: 'never_run', runComputedAt: undefined }, 'earlier_analysis'],
    ['run_state is complete_stale AND the current hash is unknown (a known kind beats an unknown value)', HASH, undefined, { runStateKind: 'complete_stale' }, 'earlier_analysis'],
    // (b) — UNKNOWN: nothing on hand says current OR not current (AI Conversation #63
    // 5825740518: after ANY reload the boot path leaves run_state absent, and
    // "Written about an earlier analysis" was then false about the latest run)
    ['run_state is unknown_degraded', HASH, HASH, { runStateKind: 'unknown_degraded', runComputedAt: undefined }, 'unknown'],
    ['no run state was stated this turn (every reload)', HASH, HASH, { runStateKind: undefined, runComputedAt: undefined }, 'unknown'],
    ['created_at is unknown', HASH, HASH, { createdAt: undefined }, 'unknown'],
    ['created_at is blank', HASH, HASH, { createdAt: '  ' }, 'unknown'],
    ['computed_at is unknown', HASH, HASH, { runComputedAt: undefined }, 'unknown'],
    ['the block hash is unknown', undefined, HASH, {}, 'unknown'],
    ['the current hash is unknown', HASH, undefined, {}, 'unknown'],
...
describe('a run_analysis card on screen — notice and action follow the failed limb', () => {
  ...
  it.each<[string, StoreInputs, Record<string, unknown>]>([
    ['the turn stated no run state (as after every reload)', { currentGraphHash: HASH, analysisState: null }, {}],
  ...
  ])('CANNOT CONFIRM when %s: never claims an earlier analysis — action inert', (_label, store, overrides) => {
    install(store)
    const card = mount(rawRunCard(overrides))
    expect(card.getAttribute('data-currency')).toBe('changed')
    expect(card.getAttribute('data-run-turn-reason')).toBe('unknown')
    expect(notice(card)).toBe("Olumi can't confirm this still matches your latest analysis.")
    expect(notice(card)).not.toMatch(/earlier|changed/i)
    expectInertBesideNotice(card)
  })
