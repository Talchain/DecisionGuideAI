      ...
      <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col space-y-4">
          <button
            type="submit"
            disabled={loading}
            className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-text-on-color bg-primary hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-info disabled:opacity-50"
          >
            {loading ? 'Sending...' : 'Send Reset Instructions'}
          </button>
          <Link
            to="/login"
            className="text-center text-sm text-info hover:text-info"
          >
            Back to Sign In
          </Link>
        </div>
