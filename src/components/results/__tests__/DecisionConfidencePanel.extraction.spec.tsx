...
describe('DecisionConfidencePanel — TriageActionCardsBody extraction regression guard', () => {
  ...
  // ── PRE-EXTRACTION BASELINE comparison ─────────────────────────────────
  //
  // The two fixtures under __fixtures__/ were generated by running the
  // *same* render scenarios against the pre-extraction `DecisionConfidencePanel.tsx`
  // (commit bf4fd022 — one commit before the extraction commit 7cd3ac2b)
  // via a one-shot capture spec in a sibling worktree. The captured HTML
  ...
  it('rich state: current rendering matches the literal pre-extraction baseline fixture', () => {
    ...
    const card = container.querySelector('[data-testid="t1-decision-confidence-card"]')
    expect(card).toBeTruthy()
    const current = normaliseHtml(card!)
    const baseline = readFileSync(
      join(__dirname, '__fixtures__', 'DecisionConfidencePanel.pre-extraction.rich.html'),
      'utf8',
    )
    expect(current).toBe(baseline)
  })

  it('sparse state: current rendering matches the literal pre-extraction baseline fixture', () => {
    const { container } = render(
      <DecisionConfidencePanel
        data={makeData({ withFragile: false, withDominant: false, withGap: false })}
        onSendMessage={() => {}}
    ...
    const current = normaliseHtml(card!)
    const baseline = readFileSync(
      join(__dirname, '__fixtures__', 'DecisionConfidencePanel.pre-extraction.sparse.html'),
      'utf8',
    )
    expect(current).toBe(baseline)
  })
