...
export interface DriverItem {
   ...
   * measured.
   *
   * Consumed only by `influenceScaleCopy`, which keys its wording on the VALUE
   * — never interpreted here. Optional: legacy fixtures and pre-stamp payloads
   * omit it, and that absence must stay silent.
   */
...
export interface UiFactorSensitivity {
   ...
   *
   * RAW producer string, NEVER narrowed — an unrecognised value must stay
   * distinguishable from an absent one, which is what lets
   * `importanceBasisTrust` fail closed. See the read in
