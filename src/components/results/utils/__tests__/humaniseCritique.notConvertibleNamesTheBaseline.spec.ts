 ...
 * the per-constraint twin, which `humaniseCritique.ts:399-403` records as
 * running on the SAME producer rules — already satisfies both halves at
 * pristine ("couldn't be compared with where its factor stands today …
 * State that factor's current level"). So at pristine this file goes RED for
 * `GOAL_THRESHOLD_NOT_CONVERTIBLE` and GREEN for its twin: the assertion is
 * demonstrably discriminating, not merely failing.
 */
 ...
 */
const NAMES_CURRENT_STANDING = /\b(current level|stands today|stands now|where it stands|today)\b/i
...
describe('the guard itself discriminates (positive control)', () => {
  ...
  it('the current-standing guard is not satisfied by silence', () => {
    ...
    expect('State the current level for your goal').toMatch(NAMES_CURRENT_STANDING)
  })
})
...
describe('the *_NOT_CONVERTIBLE family names the baseline, never the target', () => {
  ...
  it('binds to the goal code by identity, not to whatever the family happens to contain', () => {
    ...
    // The ratified 2.300 remedy is unchanged by this repair.
