...
describe('a label-only rename reaches the mounted strip (Codex 5808182879)', () => {
  it('the factor mark names the NEW label; drag-only moves change nothing', () => {
    ...
    setNodes([f])
    render(<ModelStrip isPreRun />)
    expect(screen.getByRole('button', { name: 'Inspect Vendor cost' }), 'PRECONDITION').toBeInTheDocument()
    setNodes([{ ...f, data: { ...f.data, label: 'Annual platform cost' } }])
    expect(screen.getByRole('button', { name: 'Inspect Annual platform cost' })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Inspect Vendor cost' })).toBeNull()
  })
})
