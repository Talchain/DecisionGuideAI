...
/**
 * ⛔⛔ NO FREE-NUMBER EDIT THE ENGINE CANNOT ANALYSE — witnessed live on the
 * served build (UI `3cf9fbd0`, OpenAI path, scenario `aca54686`, 24 Sep 2026).
 * This item offered "Change this value" on Enterprise customers — a unit and NO
 * cap. The reader typed 20, CEE stored `{value: 20, raw_value: 20}`, and the
 * next Run was refused: "that factor has no defined range, so the analysis
 * engine cannot interpret it". Nothing on the tab could undo it.
 *
 * ⚠ ONE CANVAS PER CASE, THE FACTOR ALONE ON IT, so the item on screen is the
 * factor under test BY KEY — and each case asserts that key before reading the
 * control, so an absence cannot come from paging to some other item. The two
 * contrasts are the shapes the gate must NOT touch; without them the witness
 * arm passes on a build that removed the control for every factor.
 */
describe('⛔ no value edit the engine cannot analyse (served witness, 24 Sep)', () => {
  const only = (factor: { id: string; type: string; data: unknown }) => {
    nodes.length = 0
    nodes.push({ id: 'g1', type: 'goal', data: { label: 'Grow margin' } }, factor)
  }
  const WITNESS = {
    id: 'f_count',
    type: 'factor',
    data: {
      ...
      observedState: { unit: 'customers', value: 0, source: 'cee_inference' },
    },
  }

  it('⛔ a unit with no cap and no recoverable frame: no edit, the reason, and the Ask act that remains', () => {
    only(WITNESS)
    const onAsk = vi.fn()
    render(<ModelReviewTool interventions={[]} onAsk={onAsk} />)
    openTool()
    expect(itemKey(), 'PRECONDITION: the witnessed factor is the item on screen').toBe('factor:f_count')
    // The value line still renders — the reader can see what is recorded…
    expect(screen.getByTestId(`${TID}-value`)).toBeInTheDocument()
    // …but no editor is offered for a number the engine could not read.
    expect(screen.queryByTestId(`${TID}-value-edit`)).toBeNull()
    expect(screen.getByTestId(`${TID}-value-edit-withheld`)).toHaveTextContent(COPY.valueEditWithheld)
    // ⭐ THE REMEDY IN THE COPY IS A CONTROL THE READER CAN PRESS, on this item.
    fireEvent.click(screen.getByTestId(`${TID}-ask`))
    expect(onAsk).toHaveBeenCalledWith(
      expect.objectContaining({ draft: COPY.askFactorDraft('Enterprise customers'), targetId: 'f_count' }),
    )
  })

  it('CONTRAST: a CAPPED factor keeps its edit — the cap is the scale', () => {
    only({
      id: 'f_capped',
      type: 'factor',
      data: {
        label: 'Seats sold',
        observedState: { cap: 200, raw_value: 49, value: 0.245, unit: 'seats', source: 'cee_inference' },
      },
    })
    render(<ModelReviewTool interventions={[]} onAsk={vi.fn()} />)
    openTool()
    expect(itemKey()).toBe('factor:f_capped')
    expect(screen.getByTestId(`${TID}-value-edit`)).toHaveAttribute('data-node-id', 'f_capped')
    expect(screen.queryByTestId(`${TID}-value-edit-withheld`)).toBeNull()
  })

  it('CONTRAST: a UNITLESS 0–1 factor keeps its edit — its field asks for a model value', () => {
    only({
      id: 'f_unitless',
      type: 'factor',
      data: { label: 'Lead time', observedState: { value: 0.7, source: 'cee_inference' } },
    })
    render(<ModelReviewTool interventions={[]} onAsk={vi.fn()} />)
    openTool()
    expect(itemKey()).toBe('factor:f_unitless')
    expect(screen.getByTestId(`${TID}-value-edit`)).toHaveAttribute('data-node-id', 'f_unitless')
    expect(screen.queryByTestId(`${TID}-value-edit-withheld`)).toBeNull()
  })

  it('CONTRAST: a capless FRAMED PAIR keeps its edit — CEE divides by the recovered frame', () => {
    // The file's own `f_ai` shape: £49 over a 0.49 level, frame 100, no cap.
    only(CANVAS[1] as { id: string; type: string; data: unknown })
    render(<ModelReviewTool interventions={[]} onAsk={vi.fn()} />)
    openTool()
    expect(itemKey()).toBe('factor:f_ai')
    expect(screen.getByTestId(`${TID}-value-edit`)).toHaveAttribute('data-node-id', 'f_ai')
