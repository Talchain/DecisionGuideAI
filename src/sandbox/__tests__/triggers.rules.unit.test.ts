...
describe('Trigger rules & payload', () => {
  ...
  it('COUNTER_GUARDRAIL_BREACH: manual overrides dynamic sigma', async () => {
    ...
    const trig = calls.find(c => c.event === 'sandbox_trigger' && c.props.rule === 'COUNTER_GUARDRAIL_BREACH')
    expect(trig).toBeTruthy()
  ...
  it('STALE_ASSUMPTION when assumptions timestamp too old', async () => {
    ...
    await vi.advanceTimersByTimeAsync(30_000); await vi.runOnlyPendingTimersAsync()
    const trig = calls.find(c => c.event === 'sandbox_trigger' && c.props.rule === 'STALE_ASSUMPTION')
    expect(trig).toBeTruthy()
  })
  ...
  it('Cooldown override when |Δp50| >= 5%', async () => {
    ...
    await vi.advanceTimersByTimeAsync(30_000); await vi.runOnlyPendingTimersAsync()
    const baseCount = calls.filter(c => c.event === 'sandbox_trigger').length

    // Within cooldown (we do not advance 24h), delta < 5% => should NOT add
    updateKRFromP50('co', 0.37)
    await vi.advanceTimersByTimeAsync(30_000); await vi.runOnlyPendingTimersAsync()
    const afterSmall = calls.filter(c => c.event === 'sandbox_trigger').length
    expect(afterSmall).toBe(baseCount)

    // Now big delta >= 5% => override cooldown and emit
    updateKRFromP50('co', 0.45)
    await vi.advanceTimersByTimeAsync(30_000); await vi.runOnlyPendingTimersAsync()
    const afterBig = calls.filter(c => c.event === 'sandbox_trigger').length
  ...
  it('CONFLICTING_IMPACTS boundary: |pos-neg| < 0.02 and (|pos|+|neg|) >= 0.08 fires; edges do not', async () => {
    ...
    await vi.advanceTimersByTimeAsync(30_000); await vi.runOnlyPendingTimersAsync()
    expect(calls.find(c => c.event === 'sandbox_trigger' && c.props.rule === 'CONFLICTING_IMPACTS')).toBeFalsy()
    ...
    await vi.advanceTimersByTimeAsync(30_000); await vi.runOnlyPendingTimersAsync()
    expect(calls.find(c => c.event === 'sandbox_trigger' && c.props.rule === 'CONFLICTING_IMPACTS')).toBeTruthy()
    ...
    await vi.advanceTimersByTimeAsync(30_000); await vi.runOnlyPendingTimersAsync()
    const hiConflicts = calls.filter(c => c.event === 'sandbox_trigger' && c.props.decisionId === 'hi' && c.props.rule === 'CONFLICTING_IMPACTS')
