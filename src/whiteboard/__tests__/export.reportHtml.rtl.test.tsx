...
describe('Export Report (HTML)', () => {
  ...
  it('exports HTML and emits telemetry', async () => {
    const ui = (
    ...
    renderSandbox(ui, { sandbox: true, exportReportHtml: true })

    const btn = await screen.findByRole('button', { name: /export report \(html\)/i })
    fireEvent.click(btn)
    ...
    expect(call?.[1]?.format).toBe('html')
  })
})
