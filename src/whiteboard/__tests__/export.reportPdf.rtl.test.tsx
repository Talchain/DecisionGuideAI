...
describe('Export Report (PDF)', () => {
  ...
  it('opens print dialog and emits telemetry', async () => {
    const ui = (
    ...
    // stub window.open to return a fake win with print
    const fakeDoc: any = { open: vi.fn(), write: vi.fn(), close: vi.fn(), title: '' }
    const printSpy = vi.fn()
    const focusSpy = vi.fn()
    const fakeWin: any = { document: fakeDoc, print: printSpy, focus: focusSpy }
    const openSpy = vi.spyOn(window, 'open').mockImplementation(() => fakeWin)

    renderSandbox(ui, { sandbox: true, exportReportPdf: true })

    const btn = await screen.findByRole('button', { name: /export report \(pdf\)/i })
    fireEvent.click(btn)
    ...
    const call = trackSpy.mock.calls.find(c => c[0] === 'sandbox_export_report')
