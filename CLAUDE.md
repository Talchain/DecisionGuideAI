...
| UI-SEM-072 | `src/canvas-vnext/vm/analysisContext.ts` + `buildOptionCard.ts` | vNext option status banding: leader identity fail-closed on `robustness.recommended_option_id` (must name a visible option node AND ≥2 option nodes; deliberately NO win-max fallback — stricter than OptionNode's isRecommended); `close_second` when win-gap to leader < GAP_THRESHOLD 0.10 (imported from buildResultsVM, shared with UI-SEM-006); unresolved leader ⇒ no leading/behind statuses anywhere on the surface | Keep — fail-closed display gate (vNext preview) |
| UI-SEM-073 | `src/canvas-vnext/vm/buildOptionCard.ts` | vNext option key-reason: parity re-derivation of OptionNode's UI-SEM-067 logic (computeBehindReason + 1e-4 leader tolerance + identical-reason suppression across losers), pinned by shared-fixture parity tests — OptionNode.tsx itself is never imported or edited (#239 churn) | Keep — parity display gate; update BOTH sites together |
| UI-SEM-074 | `src/canvas-vnext/vm/buildRelationshipCard.ts` + `vm/strings.ts` | vNext relationship why-it-matters real-signal gate (fragile-edge match or resolved-leader endpoint only, else omitted — no filler) + unified strength ladder (inspector-v2 getStrengthLabel ONLY; competing ladders locked out by importIsolation.spec, amendment A4) + confidence words from beliefExists at ≥0.70 high / ≥0.40 medium (UI-SEM-010/017 threshold precedent); Simple shows words only, Detailed appends numerals | Keep — display formatting (vNext preview) |
| UI-SEM-075 | `src/canvas-vnext/edges/edgeEncoding.ts` | vNext edge encoding: stroke width in four discrete bands (1.5/2/3/4 px) at the unified-ladder boundaries (0.20/0.40/0.70) so the picture matches the words; helps/hurts colour via `--success`/`--danger` tokens (never Goal yellow); dash reuses the LIVE binary existenceCertaintyToLineStyle rule verbatim (solid ≥0.7, dashed '6,4' below — StyledEdge's 3-tier comment is stale, never implemented); structural edges (decision→option, option→factor; StyledEdge detection mirrored) render neutral with no claims | Keep — display formatting (vNext preview) |
| UI-SEM-076 | `src/canvas-vnext/nodes/OptionNodeVNext.tsx`, `edges/RelationshipCard.tsx`, `VNextTopStrip.tsx` | vNext stale-results grey-out (Paul decision 2026-07-08): result-derived claims are RETAINED when `results_stale`, rendered dimmed (opacity-60) with an undimmed "From a previous run" marker on every affected card PLUS the top-strip "Results out of date" pill (both legs tested — amendment A7); VM carries `isStaleResult`, fields never stripped | Keep — display honesty (explicit product decision) |
| UI-SEM-077 | `src/canvas-vnext/vm/buildNodeCards.ts` (deriveFactorFlag) | vNext factor-card flag ladder: ONE flag max, priority top_driver (influenceRank 1) > could_flip (fragile edge backs the driver) > weak_evidence (evidence-gap entry names the node) > worth_checking (driver confidence < 0.40 — shared UI-SEM-010/017 low band); live flags exist only post-analysis and only for matched drivers (unmatched drivers dropped fail-closed in resultSignals.ts); 'worth_discussing' is FIXTURE-ONLY (live adapter never passes fixtureFactorFlags — pinned by noInventedClaims.spec) | Remove when CEE/PLoT provide per-factor display flags |
| UI-SEM-078 | `src/canvas-vnext/vm/buildNodeCards.ts` | vNext node-card derivations: outcome goal-effect polarity words (Helps/Drags) from computeSignedMean sign of the model's own goal-directed non-structural edge (model input, no forecast — producer sends no per-outcome distribution); risk fragile-incidence count via the canonical fragileEdgeMatch on edges touching the node (results-gated); decision lead sentence reuses UI-SEM-072 fail-closed identity + formatWinProbability, sensitivity line is a passthrough of buildResultsVM selectHinge (reused, not re-derived) | Keep — display formatting over model inputs / reused canonical derivations |
