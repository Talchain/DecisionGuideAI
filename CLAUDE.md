...
| UI-SEM-088 | `src/adapters/plot/constraintTrust.ts` (`PLOT_CONSTRAINT_NUMBERS_SUSPECT`), gated at `src/components/results/utils/selectGoalProbability.ts` (seam 1 — never substitutes the joint figure) + `src/adapters/plot/v2/responseMapper.ts` (seam 2 — omits per-option `constraint_analysis`) | **Temporary producer-defect honesty gate (2026-07-20).** PLoT's constraint normalisation is broken when a constrained node lacks explicit scale (ROADMAP 2.83, a PLoT P0): the constraint probabilities can INVERT (a VIOLATED cap → `prob_satisfied`/`probability_of_joint_goal` ≈ 1.0) and failure margins run +25–43% off; the bite is the common CEE-drafted case (no explicit scale). BLANKET suppress-until-fix at the two seams that carry constraint-derived numbers into the UI — deliberately NO client-side bite-condition detection (replicating a producer defect predicate on the consumer is the hand-maintained-mirror drift class). Seam 1: `selectGoalProbability` returns the unconstrained `goal_probability` and NEVER the joint figure while suspect (gated on the constant, not on `constraint_analysis` presence, because the live V5 path never populates per-option `constraint_analysis` so the joint arrives with no client-visible marker); this covers OptionCards goal readout, analysis-hero goalFit, OptionNode "chance of target" badge, useResultsSectionData. Seam 2: the V2 responseMapper omits per-option `constraint_analysis`, so TargetProbabilityBars, OptionCards' joint badge + "Meets/May miss all targets" verdict, the hero "and limits" wording (UI-SEM-056 reverts to goal-alone automatically) and GoalNode/OutcomeNode achievement (useNodeDisplayMetadata's inline joint copy) all take their absent-constraint branch. Presence-only copy (that a target/constraint EXISTS, no number) is untouched. Verified fail-closed clean on absence; GoalPanel readback (PR #402) already reads top-level report fields no live mapper populates and self-hides — untouched. Flip to `false` (one-line PR; pinned safe by the positive-control specs) on A1's deployed-and-verified signal | **Remove** when A1 signals the PLoT normalisation fix is deployed-and-verified (delete the constant + inline the `false` branch) |
