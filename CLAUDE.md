...
| UI-SEM-087 | `src/canvas/ui/inspector-v2/panels/GoalPanel.tsx` (`constraintsInert` gate), copy in `src/canvas/ui/inspector-v2/inspectorStrings.ts` (`GOAL_CONSTRAINT_COPY.constraintsNotUsedInAnalysis`) | Constraint display-honesty gate (same class as UI-SEM-071/082 — gate on truth, never fabricate): on the V5-canonical run path (`isV5CanonicalRunPath()` — VITE_V5_CANONICAL_ANALYSIS + VITE_ENABLE_V5_ORCHESTRATOR) the Run affordance dispatches a thin `run_analysis` chip (buildV5Payload → message + chip.action_type, no graph, no goal_constraints); the engine reads goal_constraints off its OWN server-side scenario graph, never off anything the client sends (confirmed HeroSection.tsx:91 + olumi-assistants-service chip-click dispatch), so a constraint typed/edited in this panel does not shape that run. The note is CONDITIONAL on the EXACT predicate the runner branches on — the V2 direct-run path (`isV5CanonicalRunPath()===false`, useV2Run → buildV2RunRequest) DOES send goal_constraints, so it is suppressed there. Pre-analysis only; one note shown at a time (display-surface footer when constraints exist, else entry-surface note — never stacked). Truth-telling stopgap only; routing is a held joint decision with the orchestrator workstream | Remove when the run path is unified so panel constraints reach the engine (or a producer echo confirms they were used) |
