...
| UI-SEM-054 | `src/components/results/analysis-hero/buildHeroModel.ts` | Analysis-hero outcome-axis layout domain (min/max over existing p10/p90/centres only; 5% pad, unit pad on degenerate span) — positions bars only, never displayed as data; goal threshold excluded so it cannot compress the comparison chart | Keep — display formatting (legitimate) |
...
| UI-SEM-060 | `src/components/results/analysis-hero/buildHeroModel.ts` | Analysis-hero leader-claim banding: no-goal-basis leader claims banded from producer win probabilities — ≥0.65 "most likely to be strongest overall" (top-two p10-p90 range intersection appends the overlap-advisory sentence only); ≥0.50, or win-gap over the strongest rival ≥ GAP_THRESHOLD (0.10, shared with UI-SEM-006), "slightly ahead" (runner-up "close on expected outcome" named only when top-two centres differ ≤15% of the larger magnitude); otherwise "No option is clearly ahead."; missing win probabilities fall back to the unbanded claim. Range overlap alone never produces a closeness claim | Remove when PLoT provides a leader-confidence band / close-call signal |
