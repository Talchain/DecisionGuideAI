...
| UI-SEM-060 | `src/components/results/analysis-hero/buildHeroModel.ts` | Analysis-hero close-call trigger: top-two rendered outcome rows' p10-p90 ranges intersect → tempered leader copy naming the runner-up (display calibration from existing values; never fires without both ranges) | Keep — display formatting (same class as UI-SEM-050) |
