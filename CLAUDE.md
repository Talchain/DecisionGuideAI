...
| UI-SEM-085 | `src/v5/extractPhase3FromV5Response.ts` (deriveGuidance — the single defaulting site), consumed in `src/components/results/strengthen/buildRecommendations.ts` | `deriveGuidance`: guidance-item defaults on the only live (V5) writer — `category`→`'should_fix'`, `source`→`'analysis'`, `signal_code`→`block.type`, `priority`→`50` when the producer emits neither `priority` nor `priority_rank`. These are UI-authored **severity and ordering** signals consumed downstream by `selectTopItem` (guidanceStore) and the Strengthen phase-3 band. Copy is never fabricated (title-less blocks are dropped); *rank* is. Effect when it fires: `StrengthenContainer` inverts the default to rank 50, `buildRecommendations` bands it to `phase3Base(10)+50 = 60` — ABOVE the producer-backed flip trigger (100) — and with a stable sort and identical defaults the visible "fix this first" order is the wire array order. MEASURED on the live capture `cee-response-b82c89dd-trimmed.json`: `category` and `signal_code` are defaulted on 10/10 blocks, but `priority` is NOT — CEE sends `priority_rank` on all 10, so the 50 default is narrower than assumed and fires only where CEE omits both fields (the two `exercise` blocks in `phase3-evidence-exercise.bundle-shaped.json`). Stage 1 = DISCLOSE + DEMOTE only: `priorityIsProducerSupplied` is set at the single defaulting site and carried (never recomputed) to the Strengthen consumer; defaulted rows drop to the `phase3Unranked` band (210) below the whole producer-backed ladder and each reads "Source: Olumi model review (not ranked — shown in the order received)." No replacement priority is derived from `category` (that would stack a second invention on the first). Absence of the flag is treated as unranked (fail-closed) | Remove when CEE guarantees `priority` and `category` on every guidance block — cross-repo ask filed in `parallel-briefs/UI-CROSS-REPO-ASKS-2026-07-15.md` ("guidance block priority + category"). NOTE: the ask is not landed until VERIFIED AT THE UI's OWN `@talchain/schemas` PIN — a new field is silently dropped by a consumer on an older pin, which is indistinguishable from CEE never sending it |
