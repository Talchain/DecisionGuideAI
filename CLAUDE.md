...
| UI-SEM-097 | `src/canvas/nodes/shared/factorTurningPoint.ts` (`selectFactorTurningPoint`), drawn by `src/canvas/nodes/shared/FactorFlipTrack.tsx` | **Factor-card turning point — display gate + direction-only geometry (D1a).** A factor card draws a turning-point track ONLY from a PLoT `flip_thresholds[]` row that (1) names THIS node by identity — `node_id`, else `factor_id` (the key PLoT's `flip-threshold-denormaliser.ts` emits), via `normaliseFactorFields`, never by label; (2) carries `flip_reason === 'found'` — not `flip_value !== null`, since `insufficient_precision` / `non_monotonic_grid` rows ship a value too; (3) has both endpoints finite and unequal (a partial row is dropped, never completed — the same admission `buildTippingPoints` applies). Root-wins dual read: `report.flip_thresholds` (where PLoT emits and the V5 mapper carries it) before legacy `report.robustness.flip_thresholds`. NEVER the ISL `flip_threshold` (UI-SEM-037 defaults it to 0.5). The track's two markers sit at FIXED positions ordered by the producer's two numbers — position encodes direction only, never distance; the only printed number is the flip value via the shared `formatFlipValue`. Precedence on the card at rest: turning point, else the existing prior-range line, else nothing. No arithmetic on producer values | Keep — display gate over producer data; revisit if PLoT publishes a per-factor display-scale track (e.g. `value_scale: 'display'` + bounds) that would license a proportional axis |
| UI-SEM-098 | `src/canvas/nodes/shared/nodeAttention.ts` (`ATTENTION_BUDGET`), read by `src/canvas/hooks/useNodeAttention.ts` → `BaseNode.tsx` | "Worth reviewing" display budget: at most 3 nodes carry the one aggregated attention marker, ranked by most consequential producer reason (the card's own turning point / a fragile link from the node → a top-3 VoI evidence gap → a published top-3 driver rank → a producer-mapped bias finding), then reason count, then driver rank, then id. Selection among existing producer signals only — no score, no threshold; AI-generated, Not set, Needs input and stale never trigger it; stale run-derived reasons read `Last run ·` | Keep — display budget (UI-SEM-084 class); remove when the producer ships a per-surface attention budget |
