...
- **Table grants are broad** (Supabase defaults). Both `anon` and `authenticated` roles have DML privileges on public-schema tables. This project's UI performs direct table operations via PostgREST (see "Why not revoke table-level grants?" below), so these grants must remain.
