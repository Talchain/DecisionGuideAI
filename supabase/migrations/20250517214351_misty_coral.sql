/*
  # Fix Email System and Add Email Functions
  
  1. New Functions
    - `send_email_with_template` - Sends emails using Brevo API with template support
    - `test_email_sending` - Tests email sending functionality
    - `send_team_invitation_email` - Sends team invitation emails
    
  2. Email Templates
    - Adds default email templates for test emails and team invitations
...
DECLARE
  ...
  v_key TEXT;
  v_value TEXT;
  v_cursor CURSOR FOR SELECT * FROM jsonb_each_text(p_template_data);
BEGIN
  ...
  ELSE
    ...
    IF p_template_data IS NOT NULL AND jsonb_typeof(p_template_data) = 'object' THEN
      OPEN v_cursor;
      LOOP
        FETCH v_cursor INTO v_key, v_value;
        EXIT WHEN NOT FOUND;
...
GRANT EXECUTE ON FUNCTION public.send_email_with_template TO authenticated;
GRANT EXECUTE ON FUNCTION public.send_email_with_template TO anon;
...
-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION public.test_email_sending TO authenticated;
GRANT EXECUTE ON FUNCTION public.test_email_sending TO anon;
...
-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION public.send_team_invitation_email TO authenticated;
GRANT EXECUTE ON FUNCTION public.send_team_invitation_email TO anon;
