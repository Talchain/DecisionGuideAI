/*
  # Add Invitation Logs Table and Functions
...
BEGIN
  ...
  BEGIN
    DROP POLICY IF EXISTS "Enable read for authenticated users" ON public.invitation_logs;
  EXCEPTION
  ...
  BEGIN
    DROP POLICY IF EXISTS "Enable insert for authenticated users" ON public.invitation_logs;
  EXCEPTION
    WHEN undefined_object THEN
      NULL;
  END;
