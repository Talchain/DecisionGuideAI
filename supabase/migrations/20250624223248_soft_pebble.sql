/*
  ...
  1. New Tables
    - `organisations` - Stores organization data
    - `organisation_members` - Many-to-many relationship between users and organizations

  2. Schema Changes
    - Add `organisation_id` to teams, decisions, decision_analysis, and invitations tables
  ...
  4. Security
    ...
    - Add appropriate policies for organization access control
*/
...
BEGIN
  ...
  LOOP
    ...
    -- Note: Using team_ids array instead of team_id column
    UPDATE decisions 
    SET 
    ...
    -- Update team_ids array for decisions that don't have any teams
    UPDATE decisions 
    SET team_ids = array_append(COALESCE(team_ids, '{}'::uuid[]), default_team_id)
    WHERE user_id = user_record.id 
    AND (team_ids IS NULL OR array_length(team_ids, 1) IS NULL);
...
-- Update any decisions without team_ids by assigning them to the user's first team
UPDATE decisions 
SET team_ids = ARRAY[(
  SELECT t.id 
  FROM teams t 
...
WHERE (team_ids IS NULL OR array_length(team_ids, 1) IS NULL)
AND user_id IS NOT NULL;
