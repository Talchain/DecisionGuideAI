/*
  # Add Criteria Sets Tables
  
  1. New Tables
    - `criteria_sets`: Stores reusable criteria templates
      - `id` (uuid, primary key)
      - `name` (text, unique per user/team)
      - `description` (text, optional)
      - `criteria` (jsonb, array of criteria with weights)
      - `user_id` (uuid, owner)
      - `team_id` (uuid, optional - if shared with team)
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)
      
  2. Security
    - Enable RLS
    - Add policies for personal and team-shared sets
    - Add unique constraint on name per user/team
*/
