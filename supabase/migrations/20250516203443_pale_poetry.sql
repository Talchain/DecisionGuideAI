/*
  # Email Sending Functions with Brevo API

  1. New Functions
    - `get_brevo_api_key()` - Extracts Brevo API key from SMTP URL
    - `send_email_via_brevo()` - Sends email using Brevo API
    - `send_team_invitation_email()` - Sends team invitation email
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING E'Error extracting Brevo API key: %', SQLERRM;
    RETURN NULL;
END;
$$;
...
<html>
  <body style="font-family: sans-serif; line-height:1.6; color:#333;">
    <h2>Hello ' || invitee_name || E',</h2>
    <p>' || inviter_name || E' has invited you to join the team <strong>' || team_name || E'</strong> on DecisionGuide.AI.</p>
    <p style="text-align:center; margin:40px 0;">
      <a href="' || invite_link || E'"
    ...
    <p style="word-break:break-all;"><a href="' || invite_link || E'">' || invite_link || E'</a></p>
  ...
  text_content := E'Hello ' || invitee_name || E',

' || inviter_name || E' has invited you to join the team "' || team_name || E'" on DecisionGuide.AI.
...
— The DecisionGuide.AI Team';
  ...
  SELECT
    public.send_email_via_brevo(
      to_email,
      E'You\'re invited to join "' || team_name || E'" on DecisionGuide.AI',
      html_content,
      text_content,
      'DecisionGuide.AI',
