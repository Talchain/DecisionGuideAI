...
The option's missing-percentage state therefore says “Support percentage
unavailable”, using the same metric register as the available figure; it must
not say that the whole result is unavailable or fabricate
...
This table is the consolidated scope, not a declaration that every row has shipped. Exact-head tests and deployed interaction receipts belong in [VALIDATION.md](VALIDATION.md). The [9 September restart checkpoint](../RESTART-CHECKPOINT-20260909.md) preserves the current evidence and next actions. Programme data, goal and manual-save ownership remains in the canonical programme handover; older row references below are historical carriers, not new assignments.
...
| O03 | One reference and compatible values across node, preview and inspector; option-switch freshness | Option Codex [#1353](https://github.com/Talchain/DecisionGuideAI/pull/1353), Canvas [#1343](https://github.com/Talchain/DecisionGuideAI/pull/1343), TC quantity authority | Inspector target-only/identity repair deployed as `81dbddd0`; named-reference node and currentness repair deployed as `10d060f2`. Shared scale inference remains a separate dependency; no guessed conversion. |
| O04 | Baseline label without unconditional no-change claim; full change-list recovery | Option Codex [#1342](https://github.com/Talchain/DecisionGuideAI/pull/1342) and #1353 | #1342 passed hosted gates and deployed. #1353 is deployed as `10d060f2`, extending target retention and Detailed preview recovery. Mounted acceptance outstanding. |
| O05 | Explicit missing result; preserve measured zero and existing failure/currentness | Option Codex #1342, #1353 and #1360 | Missing/zero/failed and retained-result freshness repairs deployed. #1360 is served as `2217b187`: a missing support percentage does not imply missing outcome ranges. Measured zero is retained. No new currentness browser witness. |
...
| O08 | Compact read-only inspector; neutral deltas and truthful edit affordances | Canvas [#1355](https://github.com/Talchain/DecisionGuideAI/pull/1355) | Independently reviewed and deployed as `78202540`: concise notice, explicit model values, readable read-only targets, description identity and usable navigation alongside fenced writes. Subsequent #1345/#1359 activate canonical value editing; #1365 restore acknowledgement is served as `342ab3b5`. Runtime inspect/rename/navigation/AI and restored-model save checks remain required. |
...
| G03 | Comparative vocabulary, leading-badge reservation and actual question framing | Option Codex #1362 with independent review; Canvas retains vocabulary and badge design | #1362 is served as `0d809b7b`: the Question node no longer repeats the option verdict/bar or calls a raw risk link sensitivity; reduced zoom uses the actual distinct linked-option count. Independent stability, readiness and coaching remain. #1356 comparative coaching wording is served as `b61ae1fe`. Remaining badge/vocabulary work and native acceptance are not closed by either release. |
...
| G05 | Bias registry aliases and design-system reconciliation | Canvas #1327/#1319; Option Codex #1358 with independent review | #1327 alias and #1358 observation retention are deployed; #1358 is served as `71520554`. Valid unrecognised observations keep their content and resolved references under a neutral reasoning-check heading. Four existing generic coaching prose sites and the broader #1319 reconciliation remain separate; no new bias diagnosis or native journey pass is claimed. |
| G06 | Factor influence/edge certainty/strength copy, path dimming and badge readability | Canvas, including [#1344](https://github.com/Talchain/DecisionGuideAI/pull/1344) | Evidence/measurement coaching and static evidence-gap warning deployed as `ee2b241f`. Shared metric hover/focus disclosure [#1357](https://github.com/Talchain/DecisionGuideAI/pull/1357) passed independent review and the hosted gate, and is served as `9c2923b5`. Its data and scale semantics are unchanged. Graph framing and composed native acceptance remain open. |
