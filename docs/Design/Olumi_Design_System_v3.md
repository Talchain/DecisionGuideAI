# Olumi Design System v3

**Single source of truth for all UI implementation. Supersedes Design Guidelines v1.2, Two-Shade System v2.0, and Design System v2.1.**
...
Each colour has exactly TWO shades: **Main** (text/icons/borders) + **Light** (backgrounds). Borders use `border-{colour}/30`. No extra shade tokens. Icons inherit their colour from context per §9.2.
...
**Note:** `brand.css` is authoritative for hex values. If this document and `brand.css` disagree, the CSS file wins.
...
Configure in `tailwind.config.js` to ensure these classes exist.
...
**Library:** Lucide (`lucide-react`). MIT-licensed, 24px grid, 2px stroke weight. Already in the codebase. No other icon libraries permitted — no emoji in production UI.
...
**Tier 2 — Actions (visible on hover/focus, tooltip mandatory).** Edit (`Pencil`), add evidence (`Link`), confirm (`Check`), help (`HelpCircle`), add (`Plus`), external link (`ExternalLink`). Hidden at rest, revealed on row/card hover. Keeps panels clean — think Figma layer hover pattern.
...
All four in `text-text-light` at rest, `text-text-body` on hover. Distinct icon shapes carry the differentiation without colour. Defer from PoC — revisit post-pilot when bias engagement data is available.

### 9.7 Olumi AI interaction icon

The only custom icon in the system. Marks any element where clicking triggers an AI interaction (opening the conversation panel, asking the Orchestrator about a specific element, requesting analysis).

**Design:** The Olumi logo mark (circle–square–triangle composition) rendered as a single-colour glyph at icon scale, matching Lucide's 2px stroke weight. Must be visually distinct from Lucide icons so users learn "this triggers AI."
...
- Conversation panel identity icon

Must not proliferate beyond these two contexts — if it appears everywhere, it becomes noise.

### 9.8 Space rules

- **Icon-only buttons** (with tooltip) over labelled buttons for unambiguous actions. Minimum touch target 44×44px even when the icon is 14px.
- **Icon + short label** only where the action is ambiguous (e.g. `Plus` + "Add evidence").
- **No icons in running text** or descriptions — icons in prose create visual clutter.
...
| Outcome | Ringed circle | `success` (#67C89E) | Extension of variable convention — derived result |
...
When nodes are referenced outside the canvas (panel lists, conversation blocks, search results), use the node-type icon (§9.4) rather than the shape. Icons are more legible at panel scale (14px) than geometric shapes, and they don't require SVG rendering.

---

## 11. Confidence as visual language

Confidence is communicated through a context-dependent combination of cues — border style, colour, glyph badge, and confidence bar — ensuring robustness in greyscale, print, and low-contrast conditions.
...
Confidence glyphs are rendered as text characters (not Lucide icons) inside styled badge containers. This keeps them compact and avoids icon-overload in dense contexts.

### 11.2 Context-dependent rendering

Different UI contexts use different combinations of confidence cues. The full three-cue treatment (border style + colour + glyph badge) is reserved for contexts with sufficient visual space.

| Context | Cues used | Example |
|---------|-----------|---------|
| **Canvas nodes** | Border style + colour + glyph badge | Solid green border with ✓ badge on a factor node |
| **Full-width cards** (e.g. review cards, evidence blocks) | Border style + colour + glyph badge | Dashed blue top border with ~ badge |
| **Panel driver rows** | Confidence bar + glyph badge only | Filled bar at 75% width, green, with ✓ |
| **Compact lists** (e.g. factor summaries) | Colour + glyph badge only | Green ✓ inline |
...
| Blocked | 2px `danger` (#EA7B4B) | `bg-danger-light` | `AlertTriangle` |
...
**Key differences from alerts:** left border (not top), `info-light` background (not danger/warning), Lucide `Lightbulb` icon (not `AlertTriangle`), encouraging copy ("strengthen" not "fix").

Use for: model improvement suggestions, evidence gaps, missing factors, assumption challenges. Part of the ReviewCardBlock rendering in the Orchestrator (see §15.1 for variant selection).

### 15.1 ReviewCardBlock variant selection
...
|---------|-------------|--------|------------|-------------|------|
| **Alert** (default) | Risks, critical issues, validation failures, items requiring immediate attention | Top 3px `danger` | `bg-panel` | `AlertTriangle` | Direct: "This needs attention" |
...
## 17. Loading and skeleton patterns
...
| **AI block** | Card with type-specific top border (§19.2) |
| **System event** | Centred, `text-text-light`, `panelMeta` size, no background |

### 19.2 Block rendering
...
Type-specific top borders (3px). Block type badges are small coloured dots (8px diameter, entity/semantic colour fill) positioned top-left of the block card — not icons.
...
| ReviewCardBlock | `danger` or `info` | `danger` or `info` | Alert (top border, danger) or coaching (left border, info) — see §15.1 |
...
**Block actions:** Accept → primary button (`bg-primary text-text-header`). Edit → secondary button. Dismiss → text link (`text-text-light`). Action buttons use Lucide icons per §9.3 tier 2 pattern.
...
### 19.3 Typing and tool execution
...
- **Touch targets:** minimum 44×44px (including icon-only buttons per §9.8)
...
- [ ] Tooltips present on all icon-only interactive elements (§9.8)

---

## 21. Quick reference

### 21.1 Colour quick reference
