...
- **Immutability trigger** `scenarios_workspace_immutable` (BEFORE UPDATE): value→different-value rejected (`WS010`); value→NULL permitted ONLY under the `app.wsid_deletion` sentinel (**v1.2 erratum fixed here: FK `ON DELETE SET NULL` referential actions DO fire row triggers**, so the personal-workspace deletion path must be sentinel-recognised, not assumed invisible); **NULL→value permitted ONLY in the claim shape** — same statement transitions `user_id` NULL→non-NULL AND the new `workspace_id` equals the claimer's personal workspace; any other transition → `WS011` (v1 allowed arbitrary NULL→value — fixed).
