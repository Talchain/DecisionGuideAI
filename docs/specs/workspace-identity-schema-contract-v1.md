# Workspace & Identity — Schema + Security Contract v1.3 (GATE 1 — PASS-WITH-CONDITIONS; round-3 execution-safety fixes folded)

**Status:** Gate-1 verdict = **PASS-WITH-CONDITIONS** (`A1-RULING-F3-AND-GATE1-2026-07-12.md`); v1.2 folded all four conditions + the two accepted hardenings; v1.3 folds the round-3 execution-safety review. **Gate 2 is OPEN scoped to M1+M2 authoring only; M5 (S-3) is HELD** pending CQ-6/R-1/R-2. SPEC governs the Gate-2 DDL; execution stays Paul/A1-batched.
**v1.3 (12 Jul, round-3 external review of the Gate-2 DDL — all P0/P1/P2 classes folded):** claim AUTO-STAMP in the scenarios guard trigger (v1.2's claim-shape rule was a P0 — the live user_id-only claim would strand every claim unscoped) · C2 completed: SUBJECT-scoped deletion sentinel + per-guard invariant re-assertion + ownership-writer lock rule · wsid_definer ownership BOOTSTRAP mechanics + rehearsal requirement (§5.1a) · fail-closed `role_rank` · explicit PUBLIC/anon revokes on trigger functions · exact `TO wsid_definer` policy enumeration (§5.1a) · CQ-17 owner-orphan lifecycle routed (§10) · F3 evidentiary distinction (replay file ≠ executed script; recreation rollback authored).
...
### 1.4 Helpers
`is_workspace_member(p_workspace UUID)` / `is_workspace_role(p_workspace UUID, p_min_role TEXT)` — `LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, public`, **owner `wsid_definer`**; EXECUTE → authenticated, service_role. Bodies: membership existence / `role_rank(role) >= role_rank(p_min_role)` for `auth.uid()`, **AND `role_rank(p_min_role) >= 1` (v1.3, fails closed: unknown vocabulary previously ranked 0 and authorised every member — a future-policy footgun)**. Single evolution point (org/team grants extend the member body in S-4; the RLS cutover re-points scenarios' policies here; policy shapes never change — XQ-6). `role_rank(text)`: INVOKER, IMMUTABLE. **ALL lane functions including trigger functions carry explicit `REVOKE ... FROM PUBLIC, anon` (PG default-grants EXECUTE to PUBLIC on creation).**
...
- **`auth_users_delete_precheck` (BEFORE DELETE ON auth.users, DEFINER wsid_definer) — C2:** takes `pg_advisory_xact_lock(hashtext(OLD.id::text))` (the SAME lock `create_workspace` takes for its creator — the interleave race is serialised at the DB, not in app code), then re-checks inside the deletion transaction: any surviving **non-personal** owner membership → refuse the deletion (`WM412 transfer_required`). Personal-owner rows pass. **v1.3 (C2 completed — round-3 review showed the boolean sentinel authorised too much):** the sentinel carries the **SUBJECT uuid** (`set_config('app.wsid_deletion', OLD.id::text, true)`), and every guard **re-asserts its own invariant against it** — `workspaces_delete_guard` allows only `is_personal AND created_by = subject`; `workspace_members_owner_guard` allows owner-row removal only for `user_id = subject` on a personal (or mid-cascade-deleted-under-this-sentinel) workspace. **Standing rule: every future ownership writer or transfer RPC takes the same `pg_advisory_xact_lock(hashtext(user_id::text))`.**
...
- **Guard trigger** `scenarios_workspace_immutable` (BEFORE UPDATE), v1.3: on the claim transition (`user_id` NULL→non-NULL) the trigger **AUTO-STAMPS** `workspace_id` to the claimer's personal workspace (WS020 if the provisioning invariant is breached; a caller-supplied non-personal value → WS011). **v1.2's claim-shape rule was a P0 defect** (round-3 review + automated finding): the live `claim_guest_scenario` updates `user_id` ONLY, so requiring the writer to co-set `workspace_id` would have stranded every post-M2 claim permanently unscoped — no later statement can ever satisfy `OLD.user_id IS NULL`. Auto-stamping makes today's claim work unmodified; the CEE amendment becomes defence-in-depth. Otherwise: value→different-value rejected (`WS010`); value→NULL permitted ONLY under the **subject-scoped** deletion sentinel (FK `ON DELETE SET NULL` referential actions DO fire row triggers — v1.2 erratum); any other transition → `WS011`. *Sentinel integrity assumption (recorded): JWT roles cannot pair `set_config` with DML — PostgREST offers no multi-statement transactions.*
...
### 5.1a `wsid_definer` under FORCE RLS — the policy layer that makes the owner pattern work (v1.2; bootstrap mechanics v1.3)
`wsid_definer` is a plain NOLOGIN role: it has no `BYPASSRLS` (PG15 `CREATEROLE` cannot mint one — only superusers can, and Supabase's `postgres` is not one), and FORCE RLS applies to non-owners unconditionally. Its DEFINER functions therefore need **explicit role-scoped policies** enumerated below. These policies are invisible to JWT roles, keep deny-by-default intact, and are *more* auditable than BYPASSRLS: the definer's reach is enumerated in pg_policies. The trigger guards still bind these paths — policy grants access, triggers enforce invariants.

**Ownership BOOTSTRAP (v1.3 — round-3 review: `ALTER FUNCTION ... OWNER TO` requires the executor to be able to `SET ROLE` to the new owner, and the new owner needs `CREATE` on the schema at transfer time):** M1 issues `GRANT wsid_definer TO postgres` (executor membership — also what lets M5 later `CREATE OR REPLACE` wsid_definer-owned functions) and `GRANT USAGE, CREATE ON SCHEMA public TO wsid_definer`, then **revokes CREATE after the transfers** (USAGE retained). **A rehearsal run under the real execution role is a named pre-execution requirement** — bootstrap mechanics are proven, not assumed.

**The exact `TO wsid_definer` policy set (S-1/S-2; S-3 rows land with M5):**
| Policy | Table | Cmd | USING / WITH CHECK |
|---|---|---|---|
| ws_definer_select | workspaces | SELECT | `true` / — |
| ws_definer_insert | workspaces | INSERT | — / `true` |
| ws_definer_delete | workspaces | DELETE | `true` / — (WS002 guard still binds) |
| wm_definer_all | workspace_members | ALL | `true` / `true` (WM409/WM410 guards still bind) |
| *(M5)* pp_definer_select / pp_definer_delete | person_profiles | SELECT / DELETE | `true` |
| *(M5)* pce_definer_select / pce_definer_insert | profile_consent_events | SELECT / INSERT | `true` |
...
**Still open:** #3 above (Paul) · the CQ docket (incl. the §8 data-disposition rider) · P9 · contact_consent legacy-import approval · **CQ-17 (v1.3, round-3 review): owner-orphan scenarios** — rows with `user_id` set but the auth account gone (18-row era + every future account deletion creates more via workspace SET NULL + retained authorship uuid). NOT claim-flow class (claims require `user_id NULL`); unresolvable tenant authority; blocks any future `workspace_id NOT NULL` cutover invariant. Options for Paul: quarantine / destroy / tombstone workspace / transfer / retain under a non-user subject.
