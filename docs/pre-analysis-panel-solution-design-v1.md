...
**Architecture decision — the direct DGAI to PLoT sensitivity call is an explicit temporary bridge.** CEE does not currently embed `pre_analysis_sensitivity` in any response (live-verified on both the direct draft path and the V5 turn path, 2026-06-10), so the UI's existing consumer is fed by nothing on the wire. The bridge call is flag-gated (mounts only inside the v3 panel), read-only (deterministic stateless compute, no artefacts), passthrough-safe (the existing V2 payload builder and the existing `/bff/engine/*` same-origin proxy; no service changes), and failure-tolerant (any failure leaves the store untouched; the stale-id guard plus degree fallback mean a mismatched payload can never surface). **The intended long-term home is CEE embedding sensitivity in draft output** (`analysis_ready.pre_analysis_sensitivity`, which `applyDraftResult` already ingests as of this build) — once CEE ships that, the `useSensitivityRanking` hook can retire with no other changes.
