...
if (!file) { console.error('Usage: validate-allowed-paths.js <patch>'); process.exit(2); }
const patch = fs.readFileSync(file, 'utf8');
const changed = parseChangedFiles(patch);
if (changed.length === 0) { console.log('No changed files detected in patch'); process.exit(0); }
