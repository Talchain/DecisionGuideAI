/*
  # External Collaboration Invitation Workflow
  
  1. New Tables
    - `invitations` - Stores team invitations for external users
      - `id` (uuid, primary key)
      - `email` (text, unique)
      ...
      - `decision_role` (text, check constraint)
  
  2. New Functions
    - `manage_team_invite` - Handles inviting users to teams
    - `accept_team_invitation` - Handles accepting team invitations
    - `get_pending_invitations` - Retrieves pending invitations for a team
...
CREATE TABLE IF NOT EXISTS public.invitations (
  ...
  decision_role text NOT NULL CHECK (decision_role IN ('owner', 'approver', 'contributor', 'viewer')),
