/*
  # Update RLS Policies for Organization Integration

  1. Updated Policies
    - Updated RLS policies for decisions to include organization-based access
    - Updated RLS policies for criteria_sets to include organization-based access
    - Updated RLS policies for decision_analysis to include organization-based access
    - Updated RLS policies for teams to include organization-based access

  2. Security
    - All policies maintain proper access control
    - Organization members can access appropriate resources
*/
...
CREATE POLICY "Enable read for own, team, and organization criteria sets" ON criteria_sets
  ...
  USING (
    ...
    (team_id IS NOT NULL AND EXISTS (
      SELECT 1
      FROM team_members
      WHERE team_members.team_id = criteria_sets.team_id AND team_members.user_id = uid()
    )) OR
    (team_id IS NOT NULL AND EXISTS (
      SELECT 1
      FROM teams
...
CREATE POLICY "Select own and organization teams" ON teams
  ...
  USING (
    ...
    EXISTS (
      SELECT 1
      FROM team_members
      WHERE team_members.team_id = teams.id AND team_members.user_id = uid()
    ) OR
    (organisation_id IS NOT NULL AND can_access_organisation_resource(organisation_id))
  );
